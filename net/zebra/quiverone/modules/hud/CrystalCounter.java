...
public class CrystalCounter extends HUDModule {
   private ColorSetting color = new ColorSetting("Color", this, true);

   public CrystalCounter() {
      super("CrystalCounter", "Shows how many crystals you have", new Point(600, 500), Category.HUD, true);
   }

   public void populate(Theme theme) {
      this.component = new CrystalCounter.CrystalRender(theme);
   }

   private class CrystalRender extends HUDComponent {
      int totems;

      public CrystalRender(Theme theme) {
         super(CrystalCounter.this.getName(), theme.getPanelRenderer(), CrystalCounter.this.position);
      }

      public void render(Context context) {
         super.render(context);
         int totems = Module.mc.field_1724.field_7514.field_7547.stream().filter((itemStackx) -> {
            return itemStackx.method_7909() == class_1802.field_8301;
         ...
         class_310.method_1551().field_1772.method_1720(matrixStack, String.valueOf(totems), (float)this.position.x, (float)(this.position.y - 7), CrystalCounter.this.color.getValue().getRGB());
